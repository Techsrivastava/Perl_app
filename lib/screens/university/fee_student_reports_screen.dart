...
class FeeStudentReportsScreen extends StatefulWidget {
  ...
  State<FeeStudentReportsScreen> createState() =>
      _FeeStudentReportsScreenState();
}

class _FeeStudentReportsScreenState extends State<FeeStudentReportsScreen>
  ...
  Widget _buildCourseDetailItem(String label, String value) {
    return Column(
      children: [
        Text(
          label,
          style: const TextStyle(fontSize: 8, color: AppTheme.mediumGray),
          textAlign: TextAlign.center,
        ),
  ...
  Widget _buildTotalFeesTab() {
    ...
    final filtered = studentFees.where((s) {
      return (_selectedCourse == 'All Courses' ||
              s['course'] == _selectedCourse) &&
    ...
    // Calculate totals
    final totalExp = filtered.fold<int>(
      0,
      (sum, s) => sum + (s['courseFee'] as int),
    );
    final totalRec = filtered.fold<int>(
      0,
      (sum, s) => sum + (s['paid'] as int),
    );
    final totalPend = filtered.fold<int>(
      0,
      (sum, s) => sum + (s['pending'] as int),
    );
      ...
      child: Column(
        children: [
          ...
          ...courseGroups.entries.map((entry) {
            ...
            final courseTotal = students.fold<int>(
              0,
              (sum, s) => sum + (s['paid'] as int),
            );
            final coursePending = students.fold<int>(
              0,
              (sum, s) => sum + (s['pending'] as int),
            );
            final collectionRate =
                ((courseTotal / (courseFee * students.length)) * 100)
                    .toStringAsFixed(1);
              ...
              child: Column(
                children: [
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                '${students.length} student(s) • Fee: ₹${NumberFormat('#,##,###').format(courseFee)}',
                                style: const TextStyle(
                                  fontSize: 10,
                      ...
                      children: [
                        Expanded(
                          child: _buildCourseDetailItem(
                            '💰 Expected',
                            '₹${NumberFormat('#,##,###').format(courseFee * students.length)}',
                          ),
                        ),
                        Expanded(
                          child: _buildCourseDetailItem(
                            '✅ Collected',
                            '₹${NumberFormat('#,##,###').format(courseTotal)}',
                          ),
                        ),
                        Expanded(
                          child: _buildCourseDetailItem(
                            '⏳ Pending',
                            '₹${NumberFormat('#,##,###').format(coursePending)}',
                          ),
                        ),
                      ],
                    ),
                  ...
                  ...students.map((student) {
                    final paymentPercent =
                        ((student['paid'] as int) /
                                (student['courseFee'] as int) *
                                100)
                            .toStringAsFixed(0);
                          ...
                          Row(
                            children: [
                              Container(
                                ...
                                child: Center(
                                  child: Text(
                                    student['studentName'].toString().substring(
                                      0,
                                      1,
                                    ),
                                    style: TextStyle(
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      '₹${NumberFormat('#,##,###').format(student['courseFee'])}',
                                      style: const TextStyle(
                                        fontSize: 10,
                                        fontWeight: FontWeight.bold,
                                      ),
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      '₹${NumberFormat('#,##,###').format(student['paid'])}',
                                      style: const TextStyle(
                                        fontSize: 10,
                                        color: AppTheme.success,
                                        fontWeight: FontWeight.bold,
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      '₹${NumberFormat('#,##,###').format(student['pending'])}',
                                      style: TextStyle(
                                        fontSize: 10,
                                        color: isPaid
                                            ? AppTheme.mediumGray
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      DateFormat('dd MMM').format(
                                        student['lastPayment'] as DateTime,
                                      ),
                                      style: const TextStyle(
                                        fontSize: 10,
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      student['consultancyCode'].toString(),
                                      style: const TextStyle(
                                        fontSize: 9,
                                        fontWeight: FontWeight.w500,
                                      ),
  ...
  Widget _buildPendingPaymentsTab() {
    ...
    final totalPending = mockPayments.fold<int>(
      0,
      ...
      child: Column(
        children: [
          ...
          ...courseGroups.entries.map((entry) {
            ...
            final coursePending = payments.fold<int>(
              0,
              (sum, p) => sum + (p['pendingAmount'] as int),
            );
            final coursePaid = payments.fold<int>(
              0,
                    ...
                    child: Row(
                      children: [
                        Expanded(
                          child: _buildCourseDetailItem(
                            '✅ Paid',
                            '₹${NumberFormat('#,##,###').format(coursePaid)}',
                          ),
                        ),
                        Expanded(
                          child: _buildCourseDetailItem(
  ...
  Widget _buildCommissionReportTab() {
    ...
    final totalCommission = commissionData.fold<int>(
      0,
      (sum, c) => sum + (c['commission'] as int),
    );
    final totalProfit = commissionData.fold<int>(
      0,
      (sum, c) => sum + (c['universityProfit'] as int),
    );
    final totalStudents = commissionData.fold<int>(
      0,
                            ...
                            Expanded(
                              child: Column(
                                children: [
                                  ...
                                  Text(
                                    '₹${NumberFormat('#,##,###').format(data['totalFee'])}',
                                    style: const TextStyle(
                                      fontSize: 10,
                                      fontWeight: FontWeight.bold,
                                    ),
                            ...
                            Expanded(
                              child: Column(
                                children: [
                                  ...
                                  Text(
                                    '₹${NumberFormat('#,##,###').format(data['commission'])}',
                                    style: const TextStyle(
                                      fontSize: 10,
                                      color: AppTheme.warning,
                                      fontWeight: FontWeight.bold,
                            ...
                            Expanded(
                              child: Column(
                                children: [
                                  ...
                                  Text(
                                    '₹${NumberFormat('#,##,###').format(data['universityProfit'])}',
                                    style: const TextStyle(
                                      fontSize: 10,
                                      color: AppTheme.success,
                                      fontWeight: FontWeight.bold,
      ...
      child: Column(
        children: [
          ...
          ...mockAdmissions.map((student) {
            final statusColor = student['status'] == 'Approved'
                ? AppTheme.success
                : AppTheme.warning;
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              'ID: ${student['studentId']} • ${DateFormat('dd MMM yyyy').format(student['appliedDate'] as DateTime)}',
                              style: const TextStyle(
                                fontSize: 9,
                                color: AppTheme.mediumGray,
                              ),
  ...
  Widget _buildStudentExportTab() {
    final selectedCount = [
      _selectTotalFees,
      _selectPendingPayments,
      _selectCommissionReport,
      _selectAdmissionStatus,
            ...
            child: Column(
              ...
              children: [
                ...
                _buildReportCheckbox(
                  ...
                  description:
                      'Complete fee collection details with student-wise breakdown',
                ...
                _buildReportCheckbox(
                  ...
                  description:
                      'Outstanding payments with due dates and installment details',
                  value: _selectPendingPayments,
                  onChanged: (val) =>
                      setState(() => _selectPendingPayments = val!),
                  color: AppTheme.warning,
                ),
                const SizedBox(height: 10),
                ...
                _buildReportCheckbox(
                  ...
                  description:
                      'Consultant commission breakdown and university profit margins',
                  value: _selectCommissionReport,
                  onChanged: (val) =>
                      setState(() => _selectCommissionReport = val!),
                  color: AppTheme.warning,
                ),
                const SizedBox(height: 10),
                ...
                _buildReportCheckbox(
                  ...
                  description:
                      'Student admission status with fee payment tracking',
                  value: _selectAdmissionStatus,
                  onChanged: (val) =>
                      setState(() => _selectAdmissionStatus = val!),
                  color: AppTheme.primaryBlue,
                ),
              ],
            ),
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              icon: Icon(
                Icons.download,
                size: 18,
                color: selectedCount > 0 ? AppTheme.white : AppTheme.mediumGray,
              ),
              label: Text(
                selectedCount > 0
  ...
  void _generateSelectedReports() {
    ...
    if (selectedReports.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('⚠️ Please select at least one report'),
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          '📥 Downloading ${selectedReports.length} report(s) in $_exportFormat format:\n${selectedReports.join(', ')}',
        ),
        backgroundColor: AppTheme.success,
          ...
          child: Column(
            ...
            children: [
              ...
              _buildDetailRow(
                ...
                DateFormat(
                  'dd MMM yyyy',
                ).format(student['lastPayment'] as DateTime),
              ),
